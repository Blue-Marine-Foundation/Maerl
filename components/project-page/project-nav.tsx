'use client';
import { usePathname } from 'next/navigation';
import Link from 'next/link';
import { PlusCircleIcon } from 'lucide-react';
import { useUser } from '@/components/user/user-provider';

interface NavItem {
  name: string;
  href: string;
}

export default function ProjectNavigation({
  slug,
}: Readonly<{ slug: string }>) {
  const pathname = usePathname();
  const { hasManagerAccess } = useUser();

  const items = [
    {
      name: 'Project Home',
      href: `/projects/${slug}`,
    },
    {
      name: 'Theory of Change',
      href: `/projects/${slug}/theory-of-change`,
    },
    {
      name: 'Logframe',
      href: `/projects/${slug}/logframe`,
    },
    ...(hasManagerAccess
      ? [
          {
            name: 'Updates',
            href: `/projects/${slug}/updates`,
          },
          {
            name: 'Impact',
            href: `/projects/${slug}/impact`,
          },
          {
            name: 'Archive',
            href: `/projects/${slug}/archive`,
          },
        ]
      : []),
  ];

  return (
    <nav className='flex grow items-center justify-between'>
      <ul className='flex gap-1 text-sm'>
        {items.map((item: NavItem) => {
          return (
            <li
              key={item.name}
              className={` ${pathname === item.href && 'active'} group inline-block leading-4 transition-all`}
            >
              <Link
                href={item.href}
                className='inline-block rounded-md border border-transparent px-4 py-2 text-foreground/80 transition-all ease-in-out hover:border-foreground/20 group-[.active]:border-brand-twilight/30 group-[.active]:bg-brand-twilight/10 group-[.active]:text-brand-marine'
              >
                {item.name}
              </Link>
            </li>
          );
        })}
      </ul>
      <Link
        href={`/projects/${slug}/add-update`}
        className='flex items-center gap-2 rounded-md border border-brand-urchin/30 bg-brand-urchin/15 px-3 py-1.5 text-sm text-foreground/80 transition-all hover:border-brand-urchin/50 hover:text-foreground'
      >
        <PlusCircleIcon className='h-4 w-4' /> Add update
      </Link>
    </nav>
  );
}
